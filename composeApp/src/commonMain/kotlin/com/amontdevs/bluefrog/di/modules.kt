...
private fun buildAudioRepository(notesPlayer: INotesPlayer): IAudioRepository =
    AudioRepository(notesPlayer)

private fun buildAbsoluteSessionRepository(sessionId: Int): IAbsoluteSessionRepository =
    AbsoluteSessionRepository(sessionId)

private fun buildAuthRepository(logger: IBluefrogLogger, authManager: IAuthManager): IAuthRepository =
    AuthRepository(logger, authManager)
...
private fun buildSupabaseClient(): SupabaseClient = createSupabaseClient(
    supabaseUrl = "https://sudtpqyfweqfbfhifjlu.supabase.co", //TODO: Fix hardcode values
    supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InN1ZHRwcXlmd2VxZmJmaGlmamx1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTY1NzczOTYsImV4cCI6MjA3MjE1MzM5Nn0.8LclhHt2mDDRQh_SxnBBw23NCRlYewb04FZzPOi3b2Y"
) {
    install(Auth) {
